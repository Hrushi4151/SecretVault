...
export const SecretDetailsModal = ({
  ...
  const fetchVersions = async () => {
    ...
    try {
      const response = await secretApi.getVersions(workspaceId, projectId, environmentId, secret.id);
  ...
  const handleReveal = async (versionNumber = null) => {
    ...
    try {
      ...
      setAutoMaskSeconds(15); // Auto-mask after 15 seconds for zero-leak security
    } catch (err) {
      setRevealError(err.message || 'Failed to reveal secret. Check your permissions.');
    } finally {
      ...
      <div className="relative w-full max-w-2xl bg-[#1E000A] border border-[#FFB4C8]/25 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col gap-6 text-white max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-mono text-[#A26377]">
                Environment: <span className="text-[#FFB4C8]">{environmentName}</span>
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 p-1 rounded-2xl bg-[#140007] border border-[#FFB4C8]/15 text-xs">
          ...
          >
            ...
            <span>Versions ({versions.length || secret.currentVersionNumber || 1})</span>
          </button>
            ...
            <div className="p-5 rounded-2xl bg-[#140007] border border-[#FF2D6D]/30 flex flex-col gap-4 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between pt-1">
                <span className="text-[10px] font-mono text-[#A26377]">
                  Decrypted ephemeral in-memory. Zero server-side persistence.
                </span>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono text-[#A26377]">
                Every update creates an immutable, cryptographically sealed version.
              </span>
              <button
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-[#30000F] hover:bg-[#3F0016] text-[#F4B5C8] hover:text-white border border-[#FFB4C8]/15 text-xs font-mono font-semibold transition-all flex items-center gap-1.5 self-start sm:self-auto"
                      >
                        <Unlock className="w-3 h-3 text-[#FF2D6D]" />
                        <span>Reveal v{ver.versionNumber}</span>
                      </button>
                    </div>
                  );
                })}
